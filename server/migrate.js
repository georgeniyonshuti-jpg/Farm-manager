...
async function runMigrationsOnce() {
  ...
  /** Explicit IPv4 from DNS — `connectionString` + `family` still dialed IPv6 on Render (ENETUNREACH). */
  const clientConfig = await pgClientConfigFromDatabaseUrlAsync(databaseUrl);

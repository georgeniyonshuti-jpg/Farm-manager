...
async function handleRoundCheck({ reqUser, flockId, checkinId, submittedAtIso, submissionStatus = "approved" }) {
  ...
  const status = flock
    ? ((await checkinStatusPayloadWithFcrHint(flockId, reqUser?.role ?? null, reqUser)) ?? checkinStatusPayload(flock, reqUser?.role ?? null))
    : null;
  logOpsPipeline("round_check", "schedule_recomputed", { flockId, hasStatus: Boolean(status) });
  return { payrollImpact, payrollSaved, status };
}

async function handleMortalityLog({ flockId, mortalityId, submissionStatus, role = null, auditUser = null }) {
  ...
  if (submissionStatus === "approved" && String(flockId ?? "").trim()) {
    await maybeAutoArchiveFlock(String(flockId), auditUser ? { userId: auditUser.id, role: auditUser.role } : null);
  }
...
/** @type {Map<string, { id: string, name: string, normalizedName: string }>} */
const barnNamesByNormalized = new Map();
...
/** Effective live birds for list depletion + auto-archive (matches performance summary rules). */
function effectiveBirdsLiveEstimate(flock) {
...
async function getFlockByIdForUser(flockId, user, res) {
  ...
  if (String(f.status) === "archived" && !canViewArchivedFlocks(user)) {
    res.status(404).json({ error: "Flock not found" });
    return null;
  }
  return f;
...
function assertFlockNotArchivedForMutation(f, res) {
  if (String(f.status) === "archived") {
    res.status(400).json({ error: "This flock is archived and cannot be modified." });
    return false;
  }
  return true;
}

async function maybeAutoArchiveFlock(flockId, auditCtx = null) {
  const id = String(flockId);
  const f = flocksById.get(id);
  if (!f) return;
  if (String(f.status) === "archived") return;
  const live = effectiveBirdsLiveEstimate(f);
  if (live > 0) return;
  ...
  flocksById.set(id, { ...f, status: "archived" });
...
/**
 * Upsert barn name in master list (same normalization rules as suppliers).
 * @param {string | null | undefined} rawName
 * @returns {Promise<{ id: string, name: string, normalizedName: string } | null>}
 */
async function upsertBarnByName(rawName) {
  ...
  if (hasDb()) {
    try {
      const q = await dbQuery(
        `INSERT INTO poultry_barn_names (name, normalized_name, updated_at)
         VALUES ($1, $2, now())
         ON CONFLICT (normalized_name) DO UPDATE
           SET name = EXCLUDED.name,
               updated_at = now()
      ...
      // Table may be missing during phased rollout.
    }
  }
  const existing = barnNamesByNormalized.get(normalized);
  if (existing) return existing;
  const rec = { id: `barn_mem_${crypto.randomBytes(6).toString("hex")}`, name: cleaned, normalizedName: normalized };
  barnNamesByNormalized.set(normalized, rec);
  return rec;
}

/**
 * Resolve barn from barnNameId or typed barn name.
 * @param {string | null} barnNameId
 * @param {string | null} barnName
 */
async function resolveBarnForWrite(barnNameId, barnName) {
  const bid = String(barnNameId ?? "").trim();
  if (bid && hasDb()) {
    try {
      const q = await dbQuery(
        `SELECT id::text AS id, name, normalized_name AS "normalizedName"
           FROM poultry_barn_names
          WHERE id::text = $1
          LIMIT 1`,
        [bid]
      );
      ...
      // Fallback to barnName path below.
    }
  }
  return await upsertBarnByName(barnName);
}

async function listBarnNames() {
  if (hasDb()) {
    try {
      const q = await dbQuery(
        `SELECT id::text AS id, name, normalized_name AS "normalizedName"
...
function mapDbFlockRowToMemory(row, prev = {}) {
  return {
    ...
    barnNameId: row.barnNameId != null ? String(row.barnNameId) : (prev.barnNameId ?? null),
    barnName: row.barnName != null ? String(row.barnName) : (prev.barnName ?? null),
    ...
    purchaseCostRwf:
      row.purchaseCostRwf != null && row.purchaseCostRwf !== ""
        ? Number(row.purchaseCostRwf)
        : (prev.purchaseCostRwf ?? null),
    costPerChickRwf:
      row.costPerChickRwf != null && row.costPerChickRwf !== ""
        ? Number(row.costPerChickRwf)
        : (prev.costPerChickRwf ?? null),
    purchaseSupplier:
      row.purchaseSupplier != null ? String(row.purchaseSupplier) : (prev.purchaseSupplier ?? null),
    purchaseDate:
...
async function loadFlockByIdFromDbToMemory(flockId) {
  ...
  const r = await dbQuery(
    `SELECT f.id::text AS id,
            ...
            f.barn_name_id::text AS "barnNameId",
            bn.name AS "barnName",
            f.placement_date::text AS "placementDate",
            f.initial_count AS "initialCount",
            f.target_weight_kg AS "targetWeightKg",
            f.initial_weight_kg AS "initialWeightKg",
            f.breed_code AS "breedCode",
            f.verified_live_count AS "verifiedLiveCount",
            f.verified_live_note AS "verifiedLiveNote",
            f.verified_live_at AS "verifiedLiveAt",
            f.checkin_bands AS "checkinBands",
            f.photos_required_per_round AS "photosRequiredPerRound",
            f.target_slaughter_day_min AS "targetSlaughterDayMin",
            f.target_slaughter_day_max AS "targetSlaughterDayMax",
            f.failed_reason AS "failedReason",
            f.failed_at AS "failedAt",
            f.create_draft AS "createDraft",
            f.status,
...
app.get("/api/flocks", requireAuth, requireFarmAccess, requireAnyPageAccess(["farm_flocks", "farm_checkin", "farm_feed"]), requireAction("flock.view"), async (req, res) => {
  ...
  const flocks = [...flocksById.values()]
    ...
    .filter((f) => {
      ...
      return includeDepleted || Number(f.birdsLiveEstimate ?? 0) > 0;
    })
    .map((f) => {
...
app.post("/api/flocks", requireAuth, requireFarmAccess, requirePageAccess("farm_flocks"), requireAction("flock.create"), async (req, res) => {
  ...
  let barn = null;
  try {
    barn = await resolveBarnForWrite(barnNameIdInput, barnNameInput);
  } catch (e) {
    console.error("[ERROR]", "[db] resolve barn for flock create:", e instanceof Error ? e.message : e);
  }
  if (!barn) {
    res.status(400).json({ error: "Barn name is required (barnNameId or barnName)." });
    return;
  }
        ...
        const failedInsert = await dbQuery(
          `INSERT INTO poultry_flocks
            (breed_code, placement_date, initial_count, target_weight_kg, status,
             failed_reason, failed_at, create_draft, barn_name_id)
           VALUES ($1, $2::date, $3, $4, 'failed', $5, now(), $6::jsonb,
             (SELECT id FROM poultry_barn_names WHERE normalized_name = 'unassigned' LIMIT 1))
           RETURNING id::text AS id,
                     code AS "code",
                     COALESCE(code, CONCAT('Flock ', LEFT(id::text, 8))) AS label,
                     barn_name_id::text AS "barnNameId",
                     (SELECT name FROM poultry_barn_names n WHERE n.id = barn_name_id) AS "barnName",
...
app.post("/api/flocks/:id/retry-create", requireAuth, requireFarmAccess, requireSuperuser, requirePageAccess("farm_flocks"), requireAction("flock.create"), async (req, res) => {
  ...
  let barnRetry = null;
  try {
    barnRetry = await resolveBarnForWrite(draft.barnNameId ?? null, draft.barnName ?? null);
  } catch {
    barnRetry = null;
  }
  if (!barnRetry) {
    try {
      const u = await dbQuery(
        `SELECT id::text AS id, name, normalized_name AS "normalizedName" FROM poultry_barn_names WHERE normalized_name = 'unassigned' LIMIT 1`
      );
      const ur = u.rows[0];
      if (ur?.id) {
        barnRetry = { id: String(ur.id), name: String(ur.name), normalizedName: String(ur.normalizedName) };
      }
    } catch {
      barnRetry = null;
    }
  ...
  if (!barnRetry) {
    res.status(503).json({ error: "Barn master data unavailable. Run migrations and retry." });
    return;
  }
  try {
    await ensurePoultryFlockCodeSequence();
    ...
    flocksById.set(createdId, {
      ...
      label: createdLabel,
      code: createdCode,
      barnNameId: barnRetry.id,
      barnName: barnRetry.name,
      placementDate,
      initialCount: Math.floor(initialCount),
      breedCode,
      initialWeightKg: 0,
...
app.patch("/api/flocks/:id", requireAuth, requireFarmAccess, requireSuperuser, requirePageAccess("farm_flocks"), async (req, res) => {
  ...
  try {
    ...
    const refreshed = await loadFlockByIdFromDbToMemory(id);
    appendAudit(req.authUser.id, req.authUser.role, "flock.update", "flock", id, {
      breedCode,
      placementDate,
      barnName: barn.name,
    });
...
app.patch("/api/flocks/:id/archive", requireAuth, requireFarmAccess, requireSuperuser, requirePageAccess("farm_flocks"), async (req, res) => {
  ...
  if (hasDb() && isPersistableUuid(id)) {
    try {
      await dbQuery(`UPDATE poultry_flocks SET status = 'archived', updated_at = now() WHERE id::text = $1`, [id]);
    } catch (e) {
      console.error("[ERROR]", "[flock] PATCH /api/flocks/:id/archive:", e instanceof Error ? e.message : e);
      res.status(503).json({ error: "Could not archive flock." });
      return;
    }
  }
...
app.get("/api/me/aggregate-checkin-status", requireAuth, requireFarmAccess, requireAnyPageAccess(["dashboard_laborer", "dashboard_vet", "dashboard_management"]), requireAction("flock.view"), async (req, res) => {
  try {
    ...
    if (primaryFlockId) {
      ...
      primaryStatus =
        (await checkinStatusPayloadWithFcrHint(primaryFlockId, req.authUser?.role ?? null, req.authUser))
...
app.post("/api/flocks/:id/round-checkins", requireAuth, requireFarmAccess, requirePageAccess("farm_checkin"), requireAction("flock.view"), async (req, res) => {
  ...
  const statusOut =
    derivedStatus
    ?? ((await checkinStatusPayloadWithFcrHint(f.id, req.authUser?.role ?? null, req.authUser))
      ?? checkinStatusPayload(f, req.authUser?.role ?? null));
  if (submissionStatus === "approved" && mortalityAtCheckin > 0) {
...
app.patch("/api/check-ins/:id/review", requireAuth, requireFarmAccess, requirePageAccess("farm_checkin_review"), requireVetUp, async (req, res) => {
  ...
  if (action === "approve" && mem) {
    ...
    try {
      ...
      console.error("[ERROR]", "syncMortalityEventsFromDb after check-in approve:", e instanceof Error ? e.message : e);
    }
    if (mem?.flockId) {
      await maybeAutoArchiveFlock(String(mem.flockId), { userId: req.authUser.id, role: req.authUser.role });
    }
  }
  if (action === "reject") {
...
app.post("/api/flocks/:id/slaughter-events", requireAuth, requireFarmAccess, requirePageAccess("farm_slaughter"), requireAction("slaughter.record"), requireSlaughterEventLogger, async (req, res) => {
  ...
  if (prePerf != null && birdsSlaughtered > preLive) {
    res.status(400).json({
      error: `Cannot slaughter ${birdsSlaughtered} birds — only ${preLive} birds are estimated live in flock ${f.code ?? f.label ?? f.id}. Update flock counts if this is incorrect.`,
      birdsLive: preLive,
    });
    return;
  }
  ...
  res.status(201).json({
    ...
    fcr: perf?.fcr ?? null,
    performance: perf,
    birdsLive: perf?.birdsLiveEstimate ?? null,
  });
});
...
app.get("/api/flocks/:id/performance-summary", requireAuth, requireFarmAccess, requireAnyPageAccess(["farm_flocks", "farm_checkin", "farm_feed"]), requireAction("flock.view"), async (req, res) => {
  ...
  let summary = null;
  try {
    summary = await buildFlockPerformanceSummary(f.id);
  } catch {
    res.status(503).json({ error: "Database unavailable. Please retry shortly." });
    return;
  }
...
app.patch("/api/flocks/:id/live-verification", requireAuth, requireFarmAccess, async (req, res) => {
  ...
  try {
    ...
    await maybeAutoArchiveFlock(flockId, { userId: req.authUser.id, role: req.authUser.role });
    const summary = await buildFlockPerformanceSummary(flockId);
...
app.get("/api/barn-names", requireAuth, requireFarmAccess, requireAnyPageAccess(["farm_flocks", "farm_checkin", "farm_feed"]), async (_req, res) => {
  try {
    const barns = await listBarnNames();
    res.json({ barnNames: barns.map((b) => ({ id: b.id, name: b.name })) });
  } catch (e) {
    console.error("[ERROR]", "[db] GET /api/barn-names:", e instanceof Error ? e.message : e);
    if (!hasDb()) {
      res.json({ barnNames: [...barnNamesByNormalized.values()].map((b) => ({ id: b.id, name: b.name })).sort((a, b) => a.name.localeCompare(b.name)) });
      return;
    }
    res.status(503).json({ error: "Barn name list unavailable. Please retry shortly." });
  }
});

app.post("/api/barn-names", requireAuth, requireFarmAccess, requirePageAccess("farm_flocks"), requireAction("flock.create"), async (req, res) => {
  const name = String(req.body?.name ?? "").trim();
  if (!name) {
    res.status(400).json({ error: "name is required." });
    return;
  ...
  try {
    ...
    res.status(201).json({ barnName: { id: barn.id, name: barn.name } });
  } catch (e) {
    console.error("[ERROR]", "[db] POST /api/barn-names:", e instanceof Error ? e.message : e);
    res.status(503).json({ error: "Could not create barn name." });
  }
});
...
app.get("/api/farm/ops-board", requireAuth, requireFarmAccess, requireAnyPageAccess(["dashboard_laborer", "dashboard_vet", "dashboard_management"]), requireAction("flock.view"), async (_req, res) => {
  ...
  try {
    ...
    for (const f of flocks.rows) {
      ...
      const barnFromDb = f.barnName != null && String(f.barnName).trim() !== "" ? String(f.barnName).trim() : null;
      const barn = barnFromDb ?? (label.includes("-") ? label.split("-")[0].trim() : "Unassigned");
